...
type ArrHeap[T comparable] struct {
	data []ArrHeapNode[T]
}

type ArrHeapNode[T comparable] struct {
	priority int
	Value    T
}
...
func NewArrHeap[T comparable](prealloc int) *ArrHeap[T] {
...
func (h *ArrHeap[T]) Update(val T, newPriority int) {
	for i, node := range h.data {
		if node.Value == val {
			h.data[i].priority = newPriority
			h.bubble()
			return
		}
	}
