...
func (l *LinkedListItem[T]) SetNext(next *LinkedListItem[T]) {
	l.next = next
}
...
func (l *LinkedList[T]) SetFirst(first *LinkedListItem[T]) {
	l.first = first
}
...
func (t *LinkedList[T]) Iter() <-chan *LinkedListItem[T] {
	ch := make(chan *LinkedListItem[T])
